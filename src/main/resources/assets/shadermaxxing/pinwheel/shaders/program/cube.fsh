...
{
    return screenToWorldSpace(point.xy, point.z).xyz;
}
...
void volumeVisuals(int id, vec3 lp, out vec3 color, out float baseOpacity, out float falloff)
{
    if (id == VOL_1)
    {
        vec3 black = vec3(0.0);
        vec3 white = vec3(1.0);
        vec3 gradientColor = mix(black, white, lp.x);
...
bool volumeAllowed(int id)
{
    ...
    {
        if (i == id) continue;
        if (sdf[i] < 0.0 && volumePriority(i) > p) return false;
    }
    return true;
}
